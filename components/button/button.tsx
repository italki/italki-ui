...
import Spinner from '@italki/icons/lib/spinner';
...
const ButtonTypes = tuple('default', 'primary', 'secondary', 'ghost', 'outlined', 'shadow');
      ...
      <SizeContext.Consumer>
        {size => {
          ...
          const classes = classNames(prefixCls, className, {
            ...
            [`${prefixCls}-background-outlined`]: outlined,
            [`${prefixCls}-background-ghost`]: ghost,
            [`${prefixCls}-two-chinese-chars`]: hasTwoCNChar && autoInsertSpace,
            [`${prefixCls}-block`]: block,
            [`${prefixCls}-text`]: text,
            [`${prefixCls}-lowercase`]: lowercase,
            [`${prefixCls}-rtl`]: direction === 'rtl',
          });

          let iconNode; //loading icon或其他自定义icon
          let iconLoadingColor;
          if (type === 'primary') {
            if (!outlined && !text) {
              iconLoadingColor = '#FFFFFF';
            } else {
              iconLoadingColor = '#00bfbd';
            }
          } else if (type === 'secondary') {
            if (!outlined && !text) {
              iconLoadingColor = '#FFFFFF';
            } else {
              iconLoadingColor = '#ff554b';
            }
          } else {
            iconLoadingColor = '#8C8C8C';
          }
          if (loading) {
            iconNode = (
              <span className={`${prefixCls}-icon`}>
                <Spinner stroke={iconLoadingColor} />
              </span>
            );
          } else if (icon) {
              ...
              >
                {loading ? (
                  <>
                    ...
                    <span style={{ visibility: 'hidden' }}>{kids}</span>
                  </>
                ) : (
                  <>
                    {iconNode}
