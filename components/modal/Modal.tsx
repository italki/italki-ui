...
import Cross from '@italki/icons/lib/cross';
...
export default class Modal extends React.Component<ModalProps, {}> {
  ...
  static defaultProps = {
    ...
    visible: false,
    okType: 'secondary' as ButtonType,
  };
  ...
  renderModal = ({
    ...
    const closeIconToRender = (
      <span className={`${prefixCls}-close-x`}>
        {closeIcon || <Cross className={`${prefixCls}-close-icon`} />}
      </span>
    );
    ...
    let hasNoFooter =
      (footer === '' || footer === null) &&
      !(title === '' || title === null || title === undefined);
    let hasNoTitleAndFooter =
      (title === '' || title === null || title === undefined) && (footer === '' || footer === null);
    const wrapClassNameExtended = classNames(wrapClassName, {
      [`${prefixCls}-centered`]: !!centered,
      [`${prefixCls}-wrap-rtl`]: direction === 'rtl',
      [`${prefixCls}-fullscreen`]: fullscreen,
      [`${prefixCls}-responsive`]: responsive,
      [`${prefixCls}-no-footer`]: hasNoFooter,
      [`${prefixCls}-no-title`]: hasNoTitle,
      [`${prefixCls}-no-titleAndFooter`]: hasNoTitleAndFooter,
      [`${prefixCls}-footer-noDivider`]: footerNoDivider,
    });
    ...
    return (
      <Dialog
        {...restProps}
        prefixCls={prefixCls}
        width={fullscreen ? '100%' : restProps.width}
