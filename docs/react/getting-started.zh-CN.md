...
import { ConfigProvider, DatePicker, message } from '@italki/ui';
...
```diff
- import { ConfigProvider, DatePicker, message } from '@italki/ui';
+ import { ConfigProvider, DatePicker, message, Alert } from '@italki/ui';
```
...
如果你在开发环境的控制台看到下面的提示，那么你可能使用了 `import { Button } from '@italki/ui';` 的写法引入了 antd 下所有的模块，这会影响应用的网络性能。
...
import '@italki/ui/es/button/style'; // 或者 @italki/ui/es/button/style/css 加载 css 文件
```

> 注意：antd 默认支持基于 ES module 的 tree shaking，对于 js 部分，直接引入 `import { Button } from '@italki/ui'` 也会有按需加载的效果。
...
插件会帮你转换成 `@italki/ui/es/xxx` 的写法。另外此插件配合 [style](https://github.com/ant-design/babel-plugin-import#usage) 属性可以做到模块样式的按需自动加载。
