...
import { DatePicker, message } from '@italki/ui';
...
```diff
- import { DatePicker, message } from '@italki/ui';
+ import { DatePicker, message, Alert } from '@italki/ui';
```
...
If you see logs like in the screenshot below, you might be importing all components by writing `import { Button } from '@italki/ui';`. This will affect your app's network performance.
...
```jsx
import Button from '@italki/ui/es/button';
import '@italki/ui/es/button/style'; // or @italki/ui/es/button/style/css for css format file
```

> Note: antd supports ES6 tree shaking, so `import { Button } from '@italki/ui';` will drop the js code you don't use too.

We strongly recommend using [babel-plugin-import](https://github.com/ant-design/babel-plugin-import), which can convert the following code to the '@italki/ui/es/xxx' way:
